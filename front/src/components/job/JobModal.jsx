...
const JobModal = ({isOpen,job,closeModal,setJob}) => {
  ...
  const removeSkill = (remove) => {
    ...
    const {
        ...
        onSubmit: async (values, action) => {
          try {
            const res = await api.put(`/job/edit/${job._id}`,{...values,selected})
            setJob(res.data);
            Success('Posted');
            action.resetForm()
            closeModal()
