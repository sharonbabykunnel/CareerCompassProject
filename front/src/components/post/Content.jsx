...
const Content = ({post,user,currentUserId,index,pindex,updateQuerryLike,updateQueryComment,updateQuerrySave,updateCommentReplay,userDetails,updatePostList,filter,removeRepost,updatePostCommentLike}) => {
    const dispatch = useDispatch()
    const [isExpanded, setIsExpanded] = useState(false);
    const [currentImageIndex, setCurrentImageIndex] = useState(0)
    const [isLikedByCurrentUser, setIsLikedByCurrentUser] = useState(post?.likes?.includes(currentUserId));
    const [comment, setComment] = useState('');
    const [showComment,setShowComment] = useState(false);
    const [isOpen, setIsOpen] = useState(false)
    const [showOptions, setShowOptions] = useState(false);
    const [isSaved, setIsSaved] = useState(post?.saved?.includes(currentUserId));
    ...
    const handlePostReply = async(value,mention,post,cindex) => {
        ...
        const response  = await api.post('/comment/addReply',{text:value,post,user:user.uid,mention});
          ...
          let currComment = {...response.data,user:userDetails}
          if(pindex != undefined) updateCommentReplay({pindex,index,cindex,currComment})
      }
    ...
    const uploadComment = async()=>{
        try {
            ...
            if(pindex != undefined) updateQueryComment(pindex,index,[currentComment]);
            else dispatch(updateComment({index,currentComment}))
            setComment('')
        } catch (error) {
            console.error(error);
        }
    ...
    const likeComment = useCallback(async(comment,ind,cindex)=>{
        try {
            await api.post('/comment/like',{user:userDetails?.uid,comment});
            if(pindex != undefined) updatePostCommentLike(pindex,index,cindex,userDetails?.uid)
            dispatch(updateCommentLike({user:userDetails?.uid,comment,index,ind}))
    ...
    const handleComment = async ()=>{
        ...
        if(pindex != undefined) updateQueryComment(pindex,index,response.data);
  ...
  return (
    <div className={` col-start-2 col-end-4 mt-4 p-4 rounded-xl ${post?.isBlocked ? 'bg-red-100' : 'bg-white'}`}>
        <div></div>
        <div>
            <div className='flex items-center justify-between m-4'>
                ...
                {!showOptions ? (<div className='relative ' onClick={giveOptions}><MoreHorizontal />
                
    </div>):
                 (
      <div ref={showOptionRef} className=' right-0 z-[1] mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5'>
        <div className='py-1' role='menu' aria-orientation='vertical' aria-labelledby='options-menu'>
          {user.uid === currentUserId  ? (
            <>
              <button onClick={hidePost} className='block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left' role='menuitem'>Hide Post</button>
              <button onClick={deletePost} className='block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left' role='menuitem'>Delete Post</button>
            </>)
            :
          (<>
          <button onClick={() => handleReport()} className=' px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left' role='menuitem'>Report Post</button>
          <button onClick={() => handleNotIntrested()} className=' px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left' role='menuitem'>Not Intrested</button>
          </>
        )}
        </div>
      </div>
        ...
        <div className="flex justify-between   ">
          ...
          <div key={user._id} className={`flex items-center justify-between 'bg-lite_user'  p-4 `}>
            ...
            <div><img onClick={handleComment}
                        src="https://cdn-icons-png.flaticon.com/128/1380/1380338.png" alt="" className='h-6'/>{post?.commentsCount?.length === 0 ?  '' : post?.commentsCount?.length}</div>
            <div>
            {removeRepost ? 
